...
function PagosAcademiasPage() {
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      void listarCuentasAcademia().then(setCuentas);
    }, 0);
    ...
    return cuentas.filter((item) => {
      ...
      const coincideDisciplina =
        ...
        item.inscripciones?.some(
          (i) => i.activo && i.disciplinaId === filtros.disciplinaId,
        );

      const coincideEstado =
        filtros.estado === "todos" || item.estadoCuenta === filtros.estado;
