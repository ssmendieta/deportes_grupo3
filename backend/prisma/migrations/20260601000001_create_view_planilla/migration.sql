-- Create PlanillaPagosAcademia view
-- Replaces the old physical table with a non-materialized view
-- Calculates in real-time the payment status for each deportista
...
pagos_resumen AS (
  ...
  FROM pagos
  WHERE gestion = EXTRACT(YEAR FROM CURRENT_DATE)::int
...
SELECT
  ...
  d.tipo_deportista,
  EXTRACT(YEAR FROM CURRENT_DATE)::int          AS gestion,
