-- Fix PlanillaPagosAcademia view: remove hardcoded CURRENT_DATE gestion filter
-- so the view returns data for ALL years, not just the current one.
-- The service layer filters by gestion via Prisma's WHERE clause.
...
meses_academicos AS (
  SELECT LEAST(GREATEST(EXTRACT(MONTH FROM CURRENT_DATE)::int - 2, 0), 9) AS meses_transcurridos
)
SELECT
  d.id_deportista                               AS deportista_id,
  (p.nombres || ' ' || p.ape_paterno || ' ' || COALESCE(p.ape_materno, '')) AS nombre_completo,
  d.tipo_deportista,
  COALESCE(pr.gestion, EXTRACT(YEAR FROM CURRENT_DATE)::int) AS gestion,
